...
--- 字符串分割
---@param split_string string @被分割的字符串
---@param pattern string @分隔符，可以为模式匹配
---@param search_pos_begin number @起始位置
---@param plain boolean @为true禁用pattern模式匹配；为false则开启模式匹配
...
--- 字符串连接
---@param join_table array<string>
---@param joiner string @分隔符
...
---@param plain boolean @注意：plain为true时，关闭模式匹配机制，此时函数仅做直接的 “查找子串”的操作
...
---@param s string
function string.isEmptyOrNull(s)
